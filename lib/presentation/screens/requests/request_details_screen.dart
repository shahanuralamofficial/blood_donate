...
class RequestDetailsScreen extends ConsumerWidget {
                  ...
                  Column(
                    children: [
                      if (liveRequest.status == 'donated')
                        _buildStatusAlert('রক্তদাতা জানিয়েছেন তিনি রক্ত দিয়েছেন।'),
                      ElevatedButton(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (currentDonorId != null) {
                DocumentReference donorRef = FirebaseFirestore.instance.collection('users').doc(currentDonorId);
              ...
              DocumentReference requesterRef = FirebaseFirestore.instance.collection('users').doc(req.requesterId);
          ...
          children: [
            StatefulBuilder(
              builder: (context, setState) => DropdownButton<double>(
                value: rating,
                isExpanded: true,
                items: [5.0, 4.0, 3.0, 2.0, 1.0].map((e) => DropdownMenuItem(value: e, child: Text('$e স্টার ⭐'))).toList(),
        ...
        content: Column(
          ...
          children: reasons.map((r) => RadioListTile<String>(
            ...
            onChanged: (v) async {
              ...
              await userRef.update({'totalCancelled': FieldValue.increment(1)});
              if (context.mounted) {
                Navigator.pop(context);
                Navigator.pop(context);
              }
  ...
  Widget _buildProgressCard(BloodRequestModel req) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('রক্তের প্রগ্রেস', style: TextStyle(fontWeight: FontWeight.bold)), Text('${req.donatedBags}/${req.bloodBags} ব্যাগ', style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold))]),
          const SizedBox(height: 12),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('যোগাযোগ করুন', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)), Text(req.phoneNumber, style: TextStyle(color: Colors.grey.shade600, fontSize: 13))])),
  ...
  Widget _buildStatusAlert(String msg) {
    return Container(
      ...
      child: Row(children: [const Icon(Icons.info_outline, color: Colors.orange, size: 20), const SizedBox(width: 12), Expanded(child: Text(msg, style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.orange, fontSize: 13)))]),
    );
  }
        ...
        children: [
          ...
          if (req.thankYouNote != null && req.thankYouNote!.isNotEmpty) ...[
            ...
            Container(
              padding: const EdgeInsets.all(20),
              width: double.infinity,
              decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.green.shade100)),
              child: Column(
                children: [
                  const Icon(Icons.format_quote_rounded, color: Colors.green, size: 30),
                  const SizedBox(height: 8),
                  Text(req.thankYouNote!, textAlign: TextAlign.center, style: GoogleFonts.notoSansBengali(fontSize: 15, fontStyle: FontStyle.italic, color: Colors.blueGrey.shade800, height: 1.5)),
                  const SizedBox(height: 12),
                  const Text('— গ্রহীতার পক্ষ থেকে', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildRow(String label, String value, {bool isBold = false, Color? color}) {
    return Row(
      ...
      children: [
        ...
        Text(value, style: TextStyle(fontWeight: isBold ? FontWeight.bold : FontWeight.normal, fontSize: 14, color: color ?? Colors.black87)),
      ],
    );
  }
}
